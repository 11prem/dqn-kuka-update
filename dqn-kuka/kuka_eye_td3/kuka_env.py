...
class KukaDiverseObjectEnv(KukaGymEnv):
  ...
  def __init__(self,
    ...
    p.configureDebugVisualizer(p.COV_ENABLE_SEGMENTATION_MARK_PREVIEW, 0)  # 1
    ...
    # self.observationDim = len(super(KukaDiverseObjectEnv, self).getExtendedObservation())  # added
    # self.observation_high = np.array([largeValObservation] * self.observationDim)  # added
    ...
    # self.observation_space = spaces.Box(low=-self.observation_high, high=self.observation_high)  # added
    self.observation_space = spaces.Box(low=0, high=255, shape=(self._height, self._width, 3))  # added
    self.viewer = None
  ...
  def reset(self):
    ...
    p.loadURDF("assets/plane/plane.urdf", [0.000000, 0.000000, -0.191000])
    ...
    return np.array(self._observation[0])  # prev return
  ...
  def _get_observation(self):
    ...
    # call render() in kukaGymEnv here ??
    ...
    '''
    # self.render()
    # img_arr = super(KukaDiverseObjectEnv, self).render()  # ?
    # return img_arr
    ...
    '''com_p, com_o, _, _, _, _ = p.getLinkState(self._kuka.kukaUid,
                                              self._kuka.kukaEndEffectorIndex,
                                              computeForwardKinematics=True)  # 6, 7'''
    ...
    h = self._width  # 480, 64
    w = self._height  # 640. 64
    far = 10.0
    near = 0.01
    aspect = w / h
    ...
    return rgb_array'''
    return rgb, depth, segmentation  # previous return
    # return rgb
